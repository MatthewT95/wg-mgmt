...
async function generateLANInterfaceConfig(router_id,lan_id) {
  let wgconfig = "";
  const configFiles = await fs.readdir(`data/routers/${router_id}`);
  const routerConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/router.toml`, 'utf8'));
  const lanConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/${lan_id}.lan.toml`, 'utf8'));
  let remoteConfigs = []
  // Filter for .remote.toml files in the router's directory
  for (const file of configFiles) {
    if (file.endsWith('.remote.toml')) {
      const remoteConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/${file}`, 'utf8'));
...
async function generateEndDeviceInterfaceConfig(router_id,remote_id) {
  let wgconfig = "";
  const routerConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/router.toml`, 'utf8'));
  const remoteConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/${remote_id}.remote.toml`, 'utf8'));
  const lanConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/${remoteConfig.lanId}.lan.toml`, 'utf8'));
  const lanConfigs = []
  const configFiles = await fs.readdir(`data/routers/${router_id}`);
  ...
  for (const file of configFiles) {
    if (file.endsWith('.lan.toml')) {
      const lanConfig = TOML.parse(await fs.readFile(`data/routers/${router_id}/${file}`, 'utf8'));
