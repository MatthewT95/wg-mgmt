...
async function generateLANInterfaceConfig(router_id,lan_id) {
  let wgconfig = "";
  const router_config = await TOML.parse(await fs.readFile(`data/routers/${router_id}/config.toml`, 'utf8'));
...
async function generateEndDeviceInterfaceConfig(router_id,enddevice_id) {
  ...
  wgconfig += await generateWGConfigPartialPeer(
    lanConfig.publicKey,
    `${router_config.serverDomain}:${router_config.serverPort}`,
    lanConfig.address + '/32'
  ) + '\n\n';
...
async function writeWGConfigToFile(router_id, lan_id) {
  // Get the router configuration
  let router_config = await TOML.parse(await fs.readFile(`data/routers/${router_id}/config.toml`, 'utf8'));
  if (!router_config || !router_config.interfaces || !router_config.interfaces.lans) {
    throw new Error(`Router ${router_id} configuration is invalid or missing LANs.`);
  }
  // Get the LAN configuration
  let lanConfig = router_config["interfaces"]["lans"].find(lan => lan.lanId === lan_id);
  if (!lanConfig) {
    throw new Error(`LAN with ID ${lan_id} not found in router ${router_id} configuration.`);
  }
  // Generate the WireGuard configuration
  let wgConfig = await generateLANInterfaceConfig(router_id, lan_id);
  try {
    await fs.writeFile(`/etc/wireguard/${lanConfig.wgInterface}.conf`, wgConfig);
    console.log(`WireGuard configuration for router ${router_id} written successfully.`);
  } catch (error) {
    console.error(`Error writing WireGuard configuration for router ${router_id}: ${error.message}`);
  }
}
